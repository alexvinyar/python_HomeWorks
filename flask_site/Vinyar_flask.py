...
def count_members(id_1, id_2):
    ...
    return 'Количество участников в первой группе: ' + str(members_1_num) + '<br>' + \
           ...
           'Пересекающиеся участники: ' + str(both_members)
...
def verb_page():
    if request.form:
        ...
        return render_template('verb_page.html', input=text, text=result)
...
def vk_page():
    if request.form:
        ...
        id_1 = text[0]
        id_2 = text[1]
        result = count_members(id_1, id_2)
        return render_template('vk_page.html', input=text, text=result)
